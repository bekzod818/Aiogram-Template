...
@dp.message_handler(text="♦️ Kurslar ♦️")
...
async def list_items(callback: CallbackQuery, category, subcategory, **kwargs):
    ...
    await callback.message.edit_text(text="Kurslardan birini tanlang", reply_markup=markup)
...
async def show_item(callback: CallbackQuery, category, subcategory, item_id):
    ...
    item = await db.get_course(item_id)

    if item["photo"]:
        text = f"<a href=\"{item['photo']}\">{item['coursename']}</a>\n\n"
    else:
        text = f"<b>{item['coursename']}</b>\n\n"
    text += f"<b>Narxi: {item['price']} so'm/oy</b>\n\n{item['description']}"
