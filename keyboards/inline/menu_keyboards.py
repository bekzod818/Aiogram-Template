...
# Agar kurs kategoriyasi, ost-kategoriyasi va id raqami berilmagan bo'lsa 0 ga teng bo'ladi
...
async def categories_keyboard():
    ...
    for category in categories:
        ...
        number_of_items = await db.count_courses(category["category_code"])
...
async def subcategories_keyboard(category):
    ...
    for subcategory in subcategories:
        ...
        number_of_items = await db.count_courses(
...
async def items_keyboard(category, subcategory):
    ...
    items = await db.get_courses(category, subcategory)
    for item in items:
        # Tugma matnini yasaymiz
        button_text = f"{item['coursename']} - {item['price']} so'm"
...
def item_keyboard(category, subcategory, item_id):
    ...
    markup.row(
        InlineKeyboardButton(
            text=f"📩 Ro'yhatdan o'tish", url="https://datamarkaz.uz/entry/lead?utm_source=%E2%8F%A9%20Telegram%20post%20va%20aylana%20video%20lid"
        )
    )
    markup.row(
        InlineKeyboardButton(
